...
def _fixture(db_session, suffix: str):
    ...
    model = RobotModel(model_code=f"SAM-{suffix}", manufacturer="Acme", name="Stat Scout", category="inspection")
    db_session.add_all([site, model])
    db_session.commit()
    robot = Robot(
        robot_code=f"SAR-{suffix}", name="Stat Robot", serial_number=f"SAS-{suffix}",
    ...
    db_session.commit()
    battery = Sensor(robot_id=robot.id, sensor_code=f"SA-BAT-{suffix}", sensor_type=SensorType.BATTERY, unit="percent")
    temperature = Sensor(robot_id=robot.id, sensor_code=f"SA-TMP-{suffix}", sensor_type=SensorType.TEMPERATURE, unit="celsius")
    vibration = Sensor(robot_id=robot.id, sensor_code=f"SA-VIB-{suffix}", sensor_type=SensorType.VIBRATION, unit="mm/s")
...
def test_statistical_anomalies_real_postgres_path(client, db_session):
    ...
    readings = []
    # Real varying baseline: mean is near 50 and sample standard deviation is non-zero.
    for index in range(MIN_BASELINE_SAMPLES):
        readings.append(SensorReading(
            sensor_id=battery.id, robot_id=robot.id,
            recorded_at=now - timedelta(minutes=MIN_BASELINE_SAMPLES + 1 - index),
            value=49.0 + (index % 3), source_event_id=f"sa-b-{suffix}-{index}",
        ))
    readings.append(SensorReading(
        sensor_id=battery.id, robot_id=robot.id, recorded_at=now - timedelta(seconds=30),
        value=80.0, source_event_id=f"sa-b-current-{suffix}",
    ))

    # Deliberately insufficient temperature history.
    for index in range(MIN_BASELINE_SAMPLES - 1):
        readings.append(SensorReading(
            sensor_id=temperature.id, robot_id=robot.id,
            recorded_at=now - timedelta(minutes=MIN_BASELINE_SAMPLES - index),
            value=30.0 + (index % 2), source_event_id=f"sa-t-{suffix}-{index}",
        ))
    readings.append(SensorReading(
        sensor_id=temperature.id, robot_id=robot.id, recorded_at=now - timedelta(seconds=20),
        value=31.0, source_event_id=f"sa-t-current-{suffix}",
    ))
    readings.append(SensorReading(
        sensor_id=vibration.id, robot_id=robot.id, recorded_at=now - timedelta(seconds=10),
        value=999.0, source_event_id=f"sa-v-{suffix}",
    ))
    db_session.add_all(readings)
    db_session.commit()

    response = client.get(f"/api/v1/dashboard/statistical-anomalies?robot_id={robot.id}&baseline_hours=24")
...
def test_future_reading_does_not_enter_baseline(db_session):
    ...
    for index in range(MIN_BASELINE_SAMPLES):
        readings.append(SensorReading(
            ...
            value=49.0 + (index % 3), source_event_id=f"leak-base-{suffix}-{index}",
        ))
    readings.extend([
        SensorReading(sensor_id=battery.id, robot_id=robot.id, recorded_at=as_of - timedelta(seconds=5), value=80.0, source_event_id=f"leak-current-{suffix}"),
        SensorReading(sensor_id=battery.id, robot_id=robot.id, recorded_at=as_of + timedelta(minutes=1), value=-10000.0, source_event_id=f"leak-future-{suffix}"),
    ])
    db_session.add_all(readings)
    db_session.commit()
    ...
    assert scored["baseline_mean"] > 49.0
    assert scored["baseline_mean"] < 51.0
